"""
desafio1_amanda_matheus.py — Desafio 1: Mapa de características para o Perceptron
...
Perceptron fixo e mede a acurácia. Rode:  python desafio1_amanda_matheus.py
...
class Submissao:
    ...
    def fit(self, X: torch.Tensor) -> None:
        g = torch.Generator().manual_seed(0)  # semente fixa -> phi determinística
        self.mu = X.mean(0)
        ...
        # ------------------------------------------------------------
        # Orçamento de dimensões (sempre soma exatamente DIM_MAX):
        #   1) linear + radial (r, r²)      -> sempre presentes, baratos
        #   2) quadrados + interações       -> até metade do que sobrar
        #   3) Random Fourier Features      -> o restante do orçamento
        # ------------------------------------------------------------
        fixo = d + 2
        resto = max(0, DIM_MAX - fixo)

        orcam_engenharia = resto // 2
        self.usa_quadrados = d <= orcam_engenharia
        if self.usa_quadrados:
            orcam_engenharia -= d

        pares = list(itertools.combinations(range(d), 2))[:orcam_engenharia]
        if pares:
            idx_i, idx_j = zip(*pares)
            self.idx_i = torch.tensor(idx_i, dtype=torch.long)
            self.idx_j = torch.tensor(idx_j, dtype=torch.long)
        else:
            self.idx_i = None
            self.idx_j = None

        usados_engenharia = (d if self.usa_quadrados else 0) + len(pares)
        self.rff_dim = max(0, DIM_MAX - fixo - usados_engenharia)

        if self.rff_dim > 0:
            # escala de referência: distância mediana entre pontos (heurística clássica)
            n_amostra = min(300, len(Xs))
            d2 = torch.cdist(Xs[:n_amostra], Xs[:n_amostra]) ** 2
            sigma_base = math.sqrt(d2[d2 > 0].median().item() / 2 + 1e-8)

            # multi-escala: parte das features "enxerga" detalhe fino,
            # parte enxerga padrões largos -- cobre length-scales desconhecidas
            escalas = [0.5 * sigma_base, sigma_base, 2.0 * sigma_base]
            self.W = torch.empty(d, self.rff_dim)
            self.b = torch.rand(self.rff_dim, generator=g) * 2 * math.pi

            partes_por_escala = self.rff_dim // len(escalas)
            ini = 0
            for k, sigma in enumerate(escalas):
                fim = self.rff_dim if k == len(escalas) - 1 else ini + partes_por_escala
                self.W[:, ini:fim] = torch.randn(d, fim - ini, generator=g) / sigma
                ini = fim
    ...
    def phi(self, X: torch.Tensor) -> torch.Tensor:
        ...
        if self.rff_dim > 0:
            rff = math.sqrt(2.0 / self.rff_dim) * torch.cos(Xs @ self.W + self.b)
            partes.append(rff)
